...
You can access to the app at [TUM website](https://webwing.pbs.cit.tum.de/).
...
1. Install required packages
    ...
    The web-Wing requires `einops numpy scipy tqdm` and `pytorch` to be installed. The user interface is implemented with `flask`. You can install them with `pip` or `conda`. Please be careful to the version of `pytorch` to match your local GPU.
...
3. Download trained model parameters
    ...
    The model parameters can be download from the [Tsinghua University cloud](https://cloud.tsinghua.edu.cn/f/2da76b2b4b714d31aca1/?dl=1). Unzip the download and move the folder `saves` to `<webwing folder>/`. The folder tree should look like below:
    ```text
     - webwing
     | - saves
       | - 0330_25_Run3
...
1. Start local server
    ...
    change directory to `<webwing folder>/webwing/` and run `python app.py`.
...
|-|-|-|-|-|
|CSTU| $u_i, i=0,\cdots,9$ |||These parameters defines the shape of the upper airfoil surface curve. Definition of CST parameters can be found in `cst-modeling3d`. <br> **You can also modify the airfoil shape with interactive tool by click Modify Airfoil button** |
